...
### D-013: Rick Session Surface Redesign — Triage Decisions for the RICK-UX Figma Batch
- **Decision (Storm, 2026-07-22):** (1) RICK-UX-03 is bridge-only — the post-tasting review bridge gets designed; the 7-step ReviewModal composer rework is explicitly out of batch scope. (2) Tasting sessions PROMPT a skippable re-score at completion ("Palates evolve — update your 4.3?") rather than staying score-free; the re-score feeds the bridge's pre-seeded review. (3) The completion card is shareable — it gets a real share action (reuse ReviewShare.tsx share-as-image infra). (4) Depth input (RICK-UX-02) is text-first; voice input is a later phase, and mic iconography is removed until voice actually ships.
- **Context:** Triage of the 2026-07-22 live UI/UX review of the guided tasting flow (RICK-UX-01..11, TASKS.md). Full design intent + acceptance criteria in DESIGN-BRIEF-RICK-SESSION.md.
- **Rationale:** Bridge-only keeps the batch shippable in one Claude Design session and avoids touching the composer (adjacent to CORE-IP scoring). The re-score prompt closes the loop the shelf card opens ("Palates evolve — worth another pour?") — the session that answers the question should also capture the answer. Shareable completion card: the styling already promises it and share infra exists. Text-first depth avoids designing affordances the product can't yet honor — the review's mic-icon finding showed exactly that failure mode.
