...
- **Gates:** Test — 21/21 hermetic green (7 rick-suggestions regression + 4 auth + 10 NEW rick-ask); tsc 5 before/after every slice; `npm run build` clean; 55 integration fails = pre-existing live-server dependency (documented). QA — **APPROVE-WITH-CONDITIONS, 2 BLOCKING**:
  ...
  - B2 Home.tsx guided onComplete still opens ReviewModal after bridge save (duplicate-review risk) — FIX DISPATCHED to Dev-1 agent (in flight)
...
- **Open:** land B1+B2 fixes → PM re-verify (tests/tsc/build/diff) → commit → Storm merge go (merge = prod deploy) → QA live checklist (9 items, incl. B1/B2 regression guards, Ask Rick persistence, share PNG)
