...
- **DEPLOYED & LIVE-VERIFIED (2026-07-23, Storm "merge it when B2 lands" — B2 already landed):** merge `67f98be` pushed; prod flipped to hash-matched `index-BloQLlxc.js` in ~4 min. Live checks (signed-in 375px Playwright): 1e journal provenance ✓ (session rows no score, "Rick's take + your notes · no score"); completed tap → new TastingCompletion record view via GET-only ✓ (outline ✓ hero, session card, re-score block w/ "Your review · ★4.3 · Feb 2026", empty stars, Keep-it link); bridge ✓ (kicker, not-re-scored score line, 4 seeded editable phase cards, gold Save — NOT saved, view-only check); share overlay ✓ ("Tasted with Rick · mywhiskeypedia.com", Save as image/Close); 1c session screen ✓ (text stepper "Visual · 1 of 5", no dots, no warm band, chips, one gold CTA); **Ask Rick E2E ✓** — chip → POST /api/rick/ask 200 → in-character grounded answer + hand-back, input flips to "Ask a follow-up…", exchange REHYDRATES after leave/resume (scriptJson persistence proven); journal held at 2 rows throughout (RICK-UX-01 guard); 1a mode sheet ✓ (Guide me ✓ SELECTED, Score as we go, "Remembers your last choice", gold "Pull up a chair", no mic/estimates).
- **New findings from live pass (minor, non-blocking):**
  - Audio slot renders raw `429: {"message":"Daily limit reached…"}` JSON when TTS hits the 10/day Rick House limit — needs a graceful "voice resting" state (also: per-phase TTS auto-fetch burns the daily AI budget fast — feeds the RICK-UX-05 decision)
  - Stale-cache blank page: cached index.html referenced purged assets post-deploy (module MIME errors) until hard reload — check DO/App Platform cache headers on index.html (should be no-cache); users mid-session during a deploy may hit a blank app until refresh
  - Completion card uses full Rick's Take as the italic quote for legacy sessions (quip exists in scriptJson — prefer it); bridge seeds raw instructional prose for legacy sessions (fine — user-editable; future: distilled note seeds)
- **NOT live-verified (needs a fresh session; daily AI limit consumed by testing):** full fresh tasting → bridge SAVE (create + update paths), B1 legacy re-score save, "Score as we go" → RickReviewSession live run, Home entry points. Storm smoke or next-day QA pass.
- **Session close (2026-07-23, /capture):** COMPLETE & VERIFIED — vault note `_inbox/sessions/WhiskeyPedia RICK fixes shipped and Rick Session Surfaces design implemented 2026-07-23.md`; Session Atlas reconciled (2 notes indexed incl. drained headless auto-close; re-run = 0 missing); lessons: tooling.md deploy-hash lesson reinforced w/ stale-cache flip side + new DesignSync pull-and-implement lesson, llm.md new rate-cap/auto-fire lesson; Notion Session Log `3a6ae6aa-4e5c-810e-bbf1-fe58991992ad` + project card (date/summary/Next Action/Blockers/99%) — both re-queried, no duplicates.
- **Open:** Storm smoke the save paths when the daily limit resets; RICK-UX-05 audio product call (now includes the 429 + budget angle); TastingNoteCard.tsx deletion confirmation; backlog: 429 voice-resting state, index.html cache headers, ReviewModal Mic cleanup
